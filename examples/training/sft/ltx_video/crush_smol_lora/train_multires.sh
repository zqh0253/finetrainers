...
TRAINING_DATASET_CONFIG="examples/training/sft/ltx_video/crush_smol_lora/training_multires.json"
VALIDATION_DATASET_FILE="examples/training/sft/ltx_video/crush_smol_lora/validation_multires.json"
...
# We also bump the shuffle buffer size to have ample diversity in different orders through training.
dataset_cmd=(
...
# Note: Since we're training on multiple resolutions, it might take the model more steps
# to converge. We bump up the training steps to 7500 just to be save experimentation time
# can choose the best performing checkpoint manually.
training_cmd=(
  --training_type "lora"
  --seed 42
  --batch_size 1
...
miscellaneous_cmd=(
  --tracker_name "finetrainers-ltxvideo"
